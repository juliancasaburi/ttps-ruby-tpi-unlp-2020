  ...
  module Helpers
    class BookHelper
      ...
      def self.create(book)
        PersistenceLayer::FilePersistenceLayer.create_directory(BASE_DIRECTORY, book.name)
      rescue Errno::EEXIST
        raise Exceptions::BookAlreadyExistsException, book.name
      end

      def self.rename(old_name, new_name)
        if File.directory?(RouteHelper.book_route(BASE_DIRECTORY, new_name))
          raise Exceptions::BookAlreadyExistsException, new_name
        end

        begin
          PersistenceLayer::FilePersistenceLayer.rename_directory(BASE_DIRECTORY, old_name, new_name)
        rescue Errno::ENOENT
      ...
      def self.delete_book(book_name)
        PersistenceLayer::FilePersistenceLayer.delete_directory(BASE_DIRECTORY, book_name)
      rescue Errno::ENOENT
